...
def fit_biExponential_model(arr3D_img, arr1D_b):
    ...
    for arr1D_coord in arr2D_coordBody:        
        popt, pcov = curve_fit(funcBiExp, arr1D_b[1:]-arr1D_b[0], arr3D_img[arr1D_coord[0],arr1D_coord[1],1:]/arr3D_img[arr1D_coord[0],arr1D_coord[1],0], p0=(0.15,1.5e-3,8e-3), bounds=([0, 0, 3.0e-3], [1, 2.9e-3, np.inf]), method='trf')
...
for i in range(num_cases):
    ...
    # load fitted data
    y = read_data(file_dir, fname_fit, i+1)
    
    # load tissue type data
    gt_t[:,:,i] = read_data(file_dir, fname_tissue, i+1)
...
otuput_dir = 'C:/XH/Research/dMRI/output/'
