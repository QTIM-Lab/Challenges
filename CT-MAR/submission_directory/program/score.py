...
if __name__ == "__main__":
    ...
    # submission = 'NMAR_testdata'
    folder_allresults = os.path.join(submit_dir)
    # path_submitted_data = 'submission_nmar_grig'
    path_submitted_data = os.path.join(folder_allresults)
    
    path_reference_data = os.path.join(truth_dir, '1_data_dicts', 'base.pkl')
    path_structures = os.path.join(truth_dir, '1_data_dicts', 'structures.pkl')
    path_parameters = os.path.join(truth_dir, '1_data_dicts', 'parameters.pkl')
    ...
    for i in cases:
        ...
        # ## Evaluate lesion detectability
        # print('Calculate lesion detectability metric...')
        # results[i]['lesion_detec'] = utils.metric_lesion(data_str[i]['lesion'], 
        #                                            img_1, img_2)        
    ...
    if not os.path.exists(os.path.join(submit_dir, '2_results/')):
        os.makedirs(os.path.join(submit_dir, '2_results/'))    
        
    with open(os.path.join(submit_dir, '2_results', 'results.pkl'), 'wb') as f:
    ...
    folder = os.path.join(submit_dir, '2_results')
    path_dict = os.path.join(folder, f'results.pkl')
   ...
   # Define scoring dictioary
    ...
    for i in results_submission.keys():
        ...
        # # Metric 9: Lesion detection
        # x_ctn_lesion = fit_x_dict[i]['lesion_detec']
        # fit_ctn_lesion = interpolate.interp1d(x_ctn_lesion, score_y)
        # scores[i]['lesion_detec'] = fit_ctn_lesion(
        #     results_submission[i]['lesion_detec'])



    # Export results as .txt  files(
    if not os.path.exists(os.path.join(output_dir, '3_scoring')):
        os.makedirs(os.path.join(output_dir, '3_scoring'))   
    
    for i in results_submission.keys():
        # results_list = []
        with open(os.path.join(output_dir, '3_scoring', 'results_{}.txt'.format(i)) , 'w')as f:
